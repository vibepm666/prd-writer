...
按 `references/diagnosis-guide.md`：用户 / 商业 / 开发 + 产品形态；每次 1–2 问；完成后口头总结并请用户确认。

### 第二步：概念版

按 `references/concept-template.md` 写入 `-概念版.md`。  
**唯一目的：对齐方向。** 用户 **明确确认** 前不得写落地细节。「差不多」不算确认。

### 第三步：落地版

按 `references/prd-template.md` 写入 `PRD.md`。

- **MVP 闸门**：默认只展开 🔴 核心功能的 §5；写前与用户确认 N 个 MVP 功能
...
1. **分阶段**：概念版未确认 → 不写落地 §5 细节  
2. **三视角**：方向确认前三视角均有基本答案  
